...
export async function getUserSettings(
  ...
  const result = await db.get(
    "SELECT calendar_configs, people, appearance FROM user_settings WHERE id = ?",
    [userId],
  );
  if (!result) return null;
  return {
    calendarConfigs: result.calendar_configs
      ? JSON.parse(result.calendar_configs)
      : null,
    people: result.people ? JSON.parse(result.people) : null,
    appearance: result.appearance ? JSON.parse(result.appearance) : { theme: 'light' },
  };
}
...
export async function saveUserSettings(
  ...
  const db = await getDb();
  await db.run(
    `INSERT INTO user_settings (id, calendar_configs, people, appearance)
     VALUES (?, ?, ?, ?)
    ...
    [
      userId,
      JSON.stringify(calendarConfigs || {}),
...
export async function clearAllUserSettings(): Promise<void> {
  const db = await getDb();
  await db.run(
    "UPDATE user_settings SET calendar_configs = NULL, people = NULL",
  );
}
