...
const CalendarSelectorModal: React.FC<CalendarSelectorModalProps> = ({ isOpen, onClose, calendars, calendarConfigs = {}, people = [], onSave }) => {
  const [localConfigs, setLocalConfigs] = useState<Record<string, CalendarConfig>>({});
  const [activePickerId, setActivePickerId] = useState<string | null>(null); // ID of the calendar currently picking an emoji
  ...
  const handleEmojiClick = (calendarId: string, emojiObject: EmojiClickData) => {
